...
description: >
  ...
  detection. AI-DLC v2 プラグイン開発 — contributions・manifest・tests を設計付きで作り、validate/build で検証する. Packaging over `/aidlc --scope plugin-dev`, which works
  without this skill.
...
1. `directive = bun .claude/tools/aidlc-orchestrate.ts next --scope plugin-dev $ARGUMENTS`
