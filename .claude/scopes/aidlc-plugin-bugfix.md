...
バージョン更新時は、既存の作業記述からcode-generationへ必要入力を供給できるか、承認・ビルド・回帰検証の契約が変わっていないかを確認する。変更の影響に応じて`bun run check`とこのスコープの実行経路を検証し、結果を記録してから対応基準と確認日を更新する。版番号だけを先に書き換えない。
