...
このリポジトリではフレームワークを書き換えないので回避策が要る。このスコープの
由来元である `amadeus-dlc/aidlc-grilling-plugin` では、最初の intent
（`260904-plugin-plan`）で環境変数を使って検査を切り、回避した。Units Generation を
