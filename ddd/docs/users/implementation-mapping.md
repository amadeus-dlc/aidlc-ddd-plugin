...
A reserved name has to be the whole name, not a substring of it and not one word inside it. A package name is lowercased, loses a trailing `-domain` marker and has `-` read as `_`; a TypeScript package name also drops its scope and reads `.` as `_`. A module segment is compared the same way, with `-` read as `_`. `value-objects`, `ValueObjects` and `@acme/value-objects-domain` are refused because the name itself is the classification; `identity` and `invoice-entities` are not, because they name a business term that merely ends in a reserved word. The meaning of a business term remains a review.
...
Applying re-reads and re-validates both files, so an earlier successful preview never authorises a later change. Running the command again on an applied document reports `already-migrated` and leaves the bytes as they are. A write that fails reports `write-failed` and leaves the document as it was: the new document is written to an entry of the command's own beside it and renamed over it, so the registered path holds either the bytes it had or the whole new document, never a half-written one, and a failure leaves no entry of its own behind. A mapping document that is a symbolic link is refused with `write-failed`, so the link and the file it names keep their bytes; replace it with a regular file first. A directory between the record and the document, such as `domain-design`, that is a symbolic link is refused with `write-failed` too, and the file it leads to keeps its bytes; replace it with a regular directory first. Applying rebuilds the whole document from what it read, so do not run a migration while another process is writing the same file — the other process's change would be lost.
