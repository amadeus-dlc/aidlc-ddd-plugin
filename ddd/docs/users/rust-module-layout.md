...
Hidden entries and the directories aidlc, node_modules, vendor, target, and dist are excluded from discovery. Keep third-party and generated files there; do not put application-owned source outside the inspected scope. A module declaration or Cargo target root whose source file is inside one of them — named there directly, or reached there through a symbolic link — is reported on the declaration itself rather than followed, because that file is never inspected and its layout cannot be judged. Symbolic links, crate-escaping paths, references into excluded directories, module sources not named `*.rs`, ambiguous or missing files, cfg_attr path switching, parse errors, and module-generating item macros do not pass as inspected. A declaration naming a module source not named `*.rs` is reported on the declaration, for the same reason: the discovery collects Rust sources, so that file is never inspected. Conditional compilation is not evaluated; inspection covers the declared source structure, including test-only modules. Resolve unsupported arrangements before adopting this check.
...
Module resolution reads the declarations of the bundled native extractor, so both this command and the sensor require it to be installed and launchable. It ships inside the installed DDD tools, but only for the platforms the distribution covers; on any other platform, or when the installed file is missing or altered, both stop rather than report a layout verdict — the command returns nonzero with a `reason`, and the sensor writes no verdict. Run them on a covered platform; see [native extractor distribution](../developers/native-extractor-distribution.md) for the current list and the reported conditions.
