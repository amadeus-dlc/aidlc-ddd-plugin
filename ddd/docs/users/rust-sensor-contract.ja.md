...
getterの値を使った業務判断を禁止し、ユースケース層からリポジトリの引数へ受け渡す場合は許可する。

```rust
repo.remove(invoice.id()); // 許可
let id = invoice.id();
repo.remove(id);           // 変更せず渡すローカル変数も許可

if invoice.id() == 42 { /* 業務判断なので違反 */ }
repo.remove(invoice.id() + 1); // 引数内でも計算は違反
```

センサーは、呼出し先の型がdomain/use-case層で宣言された `<Aggregate>Repository` という名前のtraitであることと、呼び出すメソッドがそのtraitに宣言されていることを確認する。`repo` という変数名や `save` というメソッド名だけでは例外にしない。明示型、`impl`/`dyn` のポート、型やimportの別名、フィールド経由の呼出しを扱う。

直接渡すほか、括弧・共有参照・不変のローカル変数を介した受け渡しを許可する。ローカル変数は、その値の利用先がすべてリポジトリの引数であることを確認する。一度でも比較・計算・加工に使う場合や、可変変数・マクロ・別の関数へ渡す場合は、この例外に含めない。ドメイン層からのgetter呼出しの制限は変わらない。

ジェネリック境界の解決、trait実装の選択、関連関数形式の呼出し、型推論が必要な受信側は、例外を証明できない。これらを含め、受け渡しを証明できないgetter呼出しはblockingのままとする。受信側の型が未解決の場合は `syntax.unresolved` の注記も出す。TypeScriptは共通の規約を適用するが、このRustセンサーで検証済みとは扱わない。
