...
移行後の文書は、このページの移行コマンド、後述の読込入口、および[実装写像](implementation-mapping.ja.md)のローダーと移行が読みます。実装写像の側はこの形式しか受け付けません。**本番センサーは `schema_version: 2` に対応していません。** 承認時に正規モデルを読み込む経路はいずれも version 1 を要求し、移行後の文書に対しては、経路ごとの規則で読込失敗を報告します。
