...
The CompilerHost reads only frozen input and pinned compiler-owned `lib.*.d.ts` assets. It performs no project or ambient `@types` discovery and does not resolve external imports/reexports. An empty settings object and a recognised project-settings payload that agrees with the request both reach extraction; any other settings become unresolved. All settings and actual extractor/parser versions participate in request identity.
