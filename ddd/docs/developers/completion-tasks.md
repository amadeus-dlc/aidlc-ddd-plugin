...
| The reports that disappeared with `analyzer.macro-opaque` | A verdict says nothing about an expression-position macro (`let value = compute_amount!();`). Nothing records that the inspection did not read through it, and the decision base is not shown to be complete without it. Whether it should carry a note, and under which rule, is undecided. The non-built-in attribute macro (`#[my_attr]`) that disappeared with it is settled by [#80](https://github.com/amadeus-dlc/aidlc-ddd-plugin/issues/80): the extractor records it as `domain-facts.unresolved` with the reason `attribute-macro`, a file rules `a` and `d` decide from that carries one stops `ddd-rust-domain` and `ddd-rust-use-case` as inspection-impossible, and any other file keeps it as a note. See [how attribute macros are handled](rust-syn-spike.md#how-attribute-macros-are-handled). | Adding a note is a new rule, which T-10-06 was not asked for, and #80 settled only the attribute macro. |
...
Completion: updated Rust declarations, migration, knowledge, stages, sensors, existing regression tests, and application behavior scenarios agree. Cover both module layouts and record the aggregate execution/persistence combinations actually tested (recorded in [the execution model and persistence verification](rust-execution-persistence-verification.md)). Existing standalone and model-execution gaps retain their own acceptance obligations.
