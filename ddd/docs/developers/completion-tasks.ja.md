...
| `analyzer.macro-opaque`とともに消えた報告 | 式の位置に書かれたマクロ（`let value = compute_amount!();`）と組込みでない属性マクロ（`#[my_attr]`）について、verdictが何も述べなくなった。いずれもモジュール宣言を隠せないため、これらが無くても判定基盤は完全だが、検査がそこを読み抜けていないことを記録するものは無い。どちらかにnoteを添えるべきか、添えるならどの規則で添えるかは未決定である。 | noteの追加は規則の新設にあたり、T-10-06の要求ではない。また、その形は属性マクロの扱いを定める[#80](https://github.com/amadeus-dlc/aidlc-ddd-plugin/issues/80)の結論と整合させる必要がある。 |
...
| 直接宣言を超えるライセンス棚卸し | [`bin/NOTICE.md`](../../tools/ddd/bin/NOTICE.md)は`ddd/experiments/rust-syn/Cargo.toml`が宣言する6件を記載しているが、`Cargo.lock`が解決する第三者クレートは21件である。残る15件（`block-buffer`・`cfg-if`・`cpufeatures`・`crypto-common`・`digest`・`generic-array`・`itoa`・`libc`・`memchr`・`serde_core`・`serde_derive`・`typenum`・`unicode-ident`・`version_check`・`zmij`）には帰属表示の記載が無い。 | `Cargo.lock`はライセンス情報を持たないため、ライセンス式は別のコマンドから取得する必要がある。加えて、これらのうち実行ファイルへリンクされるものと、ビルド時にのみ使われるものの切り分けが未確定である。両方を確定することは資産撤去ではなくライセンス棚卸しであり、T-10-06の規模の目安を超える。 |
