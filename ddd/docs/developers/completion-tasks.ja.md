...
| `analyzer.macro-opaque`とともに消えた報告 | 式の位置に書かれたマクロ（`let value = compute_amount!();`）と組込みでない属性マクロ（`#[my_attr]`）について、verdictが何も述べなくなった。検査がそこを読み抜けていないことを記録するものは無く、これらが無くても判定基盤が完全であることも示されていない。属性マクロは適用先の項目を置き換えられ、インラインモジュールの中身も書き換えうるため、そこで規則が取り落とすものは未解決である。どちらかにnoteを添えるべきか、添えるならどの規則で添えるかは未決定である。 | noteの追加は規則の新設にあたり、T-10-06の要求ではない。また、その形は属性マクロの扱いを定める[#80](https://github.com/amadeus-dlc/aidlc-ddd-plugin/issues/80)の結論と整合させる必要がある。 |
