...
[試作](rust-syn-spike.ja.md)を踏まえ、Rust＋synの採用方針は承認済み。[検査設計](inspection-contract-design.ja.md)に従い、Rustのネイティブ解析基盤と参照解決の境界を実装する。T-10-01で抽出器を製品パスへ移し配布を確立し、T-10-02で規則`a`・`d`の判定をその事実へ移して、再現した公開タプルフィールドと明示的なreturnを持つgetterの見落としを解消し、起動できない場合はこの2つのゲートを停止するようにした。T-10-03では`ddd-rust-domain`が報告する残りの規則（`b`・`c`・`g`・`domain-packaging.*`）を、その土台となる宣言索引、型解決、依存辺、パッケージ・モジュール解決とともにその事実へ移し、これによりモジュール走査とそのCI用入口も抽出器で判定するようになった。T-10-04では、`ddd-rust-use-case`と`ddd-rust-interface-adapter`だけが報告する4件の列挙（`h`・`l`・`m`・`n`）を`protocol_version` 6のもとでその事実へ移した。これにより両ゲートが報告するすべてのrule_idがその事実で判定され、出荷されるRustソースセンサーのうちモジュール配置検査以外はすべて共通契約に載った。T-10-05では、モジュール配置検査とそのCI用入口が`file`と`mod-rs`の両方でその事実により判定することを確認し、解決できない構成を合格させないためのクレート境界の回帰（宣言を書いたクレートの外へ出る`#[path]`、検査対象プロジェクトの外にあるCargo member）を追加し、[操作エラー集合の検証経路](operation-error-set.ja.md)から`file`固定を外して、写像のモジュールをそのパッケージが書かれている配置に置いて検査するようにした。T-10-06では、tree-sitter資産（`lib/rust/analyzer.ts`、同梱の`web-tree-sitter`ビルド、`tree-sitter-rust`の文法）を撤去し、出荷されるセンサー・CI用入口・ビルドした配布物が運ぶ抽出器をネイティブのものだけにして、それによって実測できたことと手放したことを記録した。T-10-07では、既存の試験・ゴールデンケース・検証シナリオがRustで実際に判定している`programming_model`×`persistence_method`×モジュール配置の組み合わせを洗い出し、根拠のケース・試験への参照つきで[実行モデルと永続化方式の検証](rust-execution-persistence-verification.ja.md)に記録し、未検証の3件（`class`×`event-sourcing`×`mod-rs`、`actor`×任意×Rustソース層、`actor`×`event-sourcing`）を理由つきで未検証として残した。判定は変更していない。ゴールデンカタログを超えるセンサー全体の互換性と、対応環境ごとの配布は未完了であり、試作でもT-10-07でもT-10が完了したわけではない。

### T-10-06からT-11へ引き継ぐ事項

いずれも資産撤去の帰結であり、T-10-06がやり残した作業ではない。何が未解決で、なぜT-10-06では閉じないかを項目ごとに示す。

| 項目 | 未解決の内容 | T-10-06で閉じない理由 |
|---|---|---|
| 残した`unverified`の項目 | `sensor answers for inputs the golden catalog does not carry`、`Cargo and cross-file resolution`、`type inference and trait solving`、`macro expansion and cfg selection`、`source edition selection`、`Linux/Windows/x86_64`、`minimum OS/Rust versions`、`WASM distribution`は[実行記録](evidence/rust-syn-spike.json)の一覧に残っている。項目ごとに、どの実測も扱えない理由は[この報告が実測した範囲](rust-syn-spike.ja.md#この報告が実測した範囲)に記載した。 | いずれも、このリポジトリに実行手段が無い実測を要するか、資産撤去ではなく配布方針として決めるプラットフォーム・パッケージングの選択である。 |
| `analyzer.macro-opaque`とともに消えた報告 | 式の位置に書かれたマクロ（`let value = compute_amount!();`）について、verdictは何も述べない。検査がそこを読み抜けていないことを記録するものは無く、それが無くても判定基盤が完全であることも示されていない。noteを添えるべきか、添えるならどの規則で添えるかは未決定である。同時に消えた組込みでない属性マクロ（`#[my_attr]`）の扱いは[#80](https://github.com/amadeus-dlc/aidlc-ddd-plugin/issues/80)で決まった。抽出器は理由`attribute-macro`の`domain-facts.unresolved`として記録し、規則`a`・`d`の判定元となるファイルにその記録があれば、`ddd-rust-domain`と`ddd-rust-use-case`は検査不能として停止する。それ以外のファイルではnoteにとどめる。詳細は[属性マクロの扱い](rust-syn-spike.ja.md#属性マクロの扱い)を参照。 | noteの追加は規則の新設にあたり、T-10-06の要求ではない。#80が決めたのは属性マクロの扱いだけである。 |
| 宣言の走査が届かない位置 | 後述の生成メソッドの写像に挙げた位置（traitの関連定数の既定値、enumのバリアントの判別値、型構文の内部に書かれた式、constジェネリックの既定値、式のパスへ書かれたconstジェネリック引数）である。各位置でtree-sitterの列挙が何を報告していたかは、保留ではなく**このリポジトリからは恒久的に答えられない**。答えを出せたのは同梱の`tree-sitter-rust.wasm`だけだったためである。 | 走査範囲を広げると、同じバッチを読む規則`c`・`d`の結論が動く。加えて、旧来の答えとの比較はここでは行えない。各位置をそれ自体の是非で判断することが残作業である。 |
| AI-DLCワークスペース記録のtree-sitter記述 | `aidlc/spaces/default/codekb/base/`は、`architecture.md`・`component-inventory.md`・`dependencies.md`・`technology-stack.md`・`api-documentation.md`・`code-quality-assessment.md`で、tree-sitterを構成要素として記述したままである。 | これらは出荷センサー・CI用入口・配布物・読者向け文書のいずれでもないワークスペースの分析記録であり、T-10-06が整合を求められた文書に含まれない。6ファイルの更新はT-10-06の規模の目安を超える。 |
...
完了条件: Rustの宣言・移行・ナレッジ・ステージ・センサー・既存の回帰テスト・実アプリの振る舞いが整合する。両モジュール配置を扱い、実際に検証した集約実行モデルと永続化方式の組み合わせを記録する（記録は[実行モデルと永続化方式の検証](rust-execution-persistence-verification.ja.md)）。単独完了とモデル実行の既存残件にも、それぞれの完了条件を適用する。
