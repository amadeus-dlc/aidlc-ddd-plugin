...
[試作](rust-syn-spike.ja.md)を踏まえ、Rust＋synの採用方針は承認済み。[検査設計](inspection-contract-design.ja.md)に従い、Rustのネイティブ解析基盤と参照解決の境界を実装する。再現した公開タプルフィールドと明示的なreturnを持つgetterの見落としも修正する。名前・型の解決実装、全センサーの互換性、対応環境ごとの配布は未完了であり、試作によってT-10が完了したわけではない。
