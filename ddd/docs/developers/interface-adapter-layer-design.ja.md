...
CQRSのコマンド側はドメイン・ユースケース・インターフェイスアダプタ、クエリ側はユースケース・インターフェイスアダプタで構成する。クエリ側に更新用ドメイン層を設けず、DAOを通してDTOを取得する。

コマンド側のインターフェイスアダプタ層は更新コントローラ、リポジトリ実装、外部クライアントを持つ。クエリ側のインターフェイスアダプタ層は取得コントローラとDAO実装を持つ。非CQRSでも、永続化アダプタ・外部クライアント・薄い入出力変換という責務は維持する。
...
クエリ側は更新用集約・ドメイン型・リポジトリポートを参照しない。更新の不変条件を複製せず、検索・表示モデルの整形を担う。クエリ側のインターフェイスアダプタ層からクエリユースケースへ依存する。

RMUはコマンド側のインターフェイスアダプタ層やクエリ側のインターフェイスアダプタ層に兼務させず独立させる。中間状態の表示範囲はユースケース設計に合わせる。
...
CQRS分離、ポートの責務、復元、RMU、外部モデルとの境界変換を扱う。DB/RPCクライアントはインターフェイスアダプタに置き、本プラグインのinfrastructure層には言語拡張だけを置く。この層名の定義を明示して使う。
