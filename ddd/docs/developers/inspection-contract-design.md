...
Delivery is split into separate intents under [parent Issue #34](https://github.com/amadeus-dlc/aidlc-ddd-plugin/issues/34); see the [T-09 child table](completion-tasks.md#t-09-define-the-common-contracts-and-migrate-artifacts). This document defines the shared destination, not the scope of one intent. Each child closes on its own stated outcome and verification. Broader language/runtime support stays in T-10/T-11; newly discovered prerequisites are recorded in the parent.
