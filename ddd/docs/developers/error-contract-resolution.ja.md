...
参照指定子がどのファイルを指すかは、検査時にパッケージマニフェストを読み直すのではなく、記録した入口から答えます。bare指定子は公開された subpath でパッケージへ到達し、相対指定子はスナップショット内のファイルへ到達します。ファイル自体が見つかっても、参照元のプロジェクト参照がそのパッケージを持たない場合は `invalid-project-reference` として拒否します。参照が解決できることと、その参照が許されることは別の問いです。
