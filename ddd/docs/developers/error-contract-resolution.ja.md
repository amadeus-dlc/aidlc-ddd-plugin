...
依存は、記載されたversionがこのプロジェクトの持つパッケージを受け入れる場合にのみ記録します。この境界が扱う選択子は、workspaceプロトコル（`workspace:*`、`workspace:^`、`workspace:~`、および `workspace:` に続く厳密なversion）と、厳密なversionそのものです。いずれの場合も、workspaceパッケージが宣言するversionと一致している必要があります。範囲指定、比較子の並び、タグ、レジストリ別名、リンクは、このプロジェクトがたまたま持つversionを受け入れるものとは読まず、`unavailable` として拒否します。範囲がどのversionを受け入れるかは、この境界が答える問いではないためです。この拒否はプロジェクト設定の性質であり、条件が既に同名の複数versionを記録しているときに参照が報告する `unsupported-version-resolution` とは別のものです。
