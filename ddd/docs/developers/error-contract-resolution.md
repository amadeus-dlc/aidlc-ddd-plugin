...
Which file a specifier names is answered from the recorded entry points rather than from a package manifest read again at inspection time. A bare specifier reaches a package by its published subpath; a relative specifier reaches a file of the snapshot. Reaching a package the importer's project references do not carry is refused as `invalid-project-reference` even when the file itself was found, so resolving a reference and being allowed to make it stay separate questions.
