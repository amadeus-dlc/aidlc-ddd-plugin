...
The shipped configuration is `.ddd.toml` with `schema_version = 2`, `languages`, and one table per language in use — `[rust] module_layout = "file" | "mod-rs"`, and the TypeScript keys the same document defines. A document still in `schema_version = 1`, which named the Rust layout and nothing else, is refused until it is migrated. Declaring TypeScript in this document does not make TypeScript generated or inspected. See the [project settings](../users/project-settings.md) for the format and the [current Rust contract](../users/rust-module-layout.md) for what the Rust axis means.
...
T-09-09 connects the four formats to the paths that already read those artifacts. Every production gate now reads `schema_version: 2` of the canonical model, the aggregate mapping and the layer declaration, and `schema_version = 2` of the project settings; the generation instructions, the sensor manifests and the fixtures state the same formats. A record still in the format it has to be migrated from is refused rather than read as the format it is not, and `ddd-artifact-set migrate` converts a project's settings, model, mapping and layer declarations in one step, checking them against each other before it writes anything. See [artifact set migration](../users/artifact-migration.md). The Rust source sensors keep their existing inspection scope: they receive the Rust entries of the mapping, projected onto the crate name and module path they compare, and nothing placed in another language reaches them. TypeScript remains ungenerated and uninspected.
...
| Implementation mappings | Closed in T-09-06 and T-09-09. The [aggregate mapping reader](../../tools/ddd/lib/aggregate-mapping/index.ts) reads `schema_version: 2`, which records each language's location under `code`, and migrates a crate/module document explicitly; every production gate reads it through that reader, and the [Rust view](../../tools/ddd/lib/rules/rust/mapping.ts) projects its Rust entries onto the crate name and module path the Rust rules compare. The crate-fixed [declaration reader](../../tools/ddd/lib/sensors/declaration.ts) is a migration source only. | Match the mapping's names against resolved code symbols. |
