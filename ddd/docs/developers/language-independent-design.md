...
Exact schema versions, field names, binding syntax, migration command arguments, and migration validation details remain implementation-design work. There is no implemented migration command for this agreement yet.
...
| Implementation mappings | crate fields, `::` module paths, and the `crate` root convention are built into [declarations](../../tools/ddd/lib/sensors/declaration.ts) and [package validation](../../tools/ddd/lib/packaging/declarations.ts). | Common mapping semantics and explicit migration, with native location details preserved per language. |
| Method errors | Model declarations and references are checked; the [domain sensor](../../tools/ddd-sensor-rust-domain.ts) does not compare Result error types and their variant sets with each method's errors. | Add method-specific error-type and set checks in Rust and TypeScript. |
| Error owner consistency | [Loader](../../tools/ddd/lib/schema/loader.ts) checks reference existence and ID ownership, but code inspection found no direct equality check between `DomainError.command` and its containing command. | Add a reproduction and close the ownership check; retain this as an identified implementation gap, not a previously verified guarantee. |
| Generation errors | [FactoryRule](../../tools/ddd/lib/schema/model.ts) has target/preconditions, without an error set and method binding. | Add canonical generation errors and corresponding code bindings/checks. |
