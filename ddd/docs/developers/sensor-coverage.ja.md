...
対象は10センサー・73規則項目。71項目は対応する所見を直接検証し、2項目はローダーによる先行拒否を検証する。配布物では359ケースを各環境で実行する。承認経路には、この表から選んだ154入力を各環境で通す。
...
| ddd-rust-domain<br>`d` | `clean-self-getter` | `violation-d`<br>`violation-d-split-getter`<br>`violation-d-repository-domain-layer` | `violation-d-split-getter`<br>self呼出しは許可し、別ファイルの受信型も照合する。 |
...
| ddd-rust-use-case<br>`d` | `clean-d-repository-argument` | `violation-d-getter-alias`<br>`violation-d-repository-business-branch`<br>`violation-d-repository-arithmetic`<br>`violation-d-repository-argument-branch`<br>`violation-d-repository-transformation`<br>`violation-d-repository-local-business-use`<br>`violation-d-repository-local-unused`<br>`violation-d-repository-local-mutable`<br>`violation-d-repository-local-macro`<br>`violation-d-repository-local-closure`<br>`violation-d-repository-mutable-borrow`<br>`violation-d-repository-unknown-method`<br>`violation-d-repository-local-repo-shadow`<br>`violation-d-repository-unrelated-function`<br>`violation-d-repository-unrelated-port`<br>`violation-d-repository-mixed-consumers`<br>`violation-d-repository-mixed-getters`<br>`violation-d-repository-local-shadow-business`<br>`violation-d-repository-local-shadow-initializer`<br>`violation-d-repository-name-collision` | `violation-d-repository-local-business-use`<br>リポジトリ引数への受け渡しを許可し、業務判断・加工・別の利用先は拒否する。 |
