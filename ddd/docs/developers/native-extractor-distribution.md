...
Distributing the TypeScript extractor and moving the Rust crate out of `experiments/rust-syn/` are not part of this change. Rules `a` and `d` were connected in T-10-02, T-10-03 connected the rest of what `ddd-rust-domain` reports along with the package and module resolution its packaging rules read, T-10-04 connected the four rules only `ddd-rust-use-case` and `ddd-rust-interface-adapter` report — `h`, `l`, `m`, `n` — and T-10-05 held the module-layout inspection and its CI entry to the same facts in both project layouts. T-10-06 removed the tree-sitter assets, so this is the only extractor any shipped Rust sensor reads. The third-party crates its crate declares directly are listed in [`bin/NOTICE.md`](../../tools/ddd/bin/NOTICE.md), which ships beside the executable and records that those are not the whole set a build resolves.
