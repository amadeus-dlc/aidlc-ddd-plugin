...
readonlyを含め、ドメインの状態は非公開にします。Domain PrimitiveとValue Objectは不変とし、生成時に検証します。完全な生成経路を使い、空の状態を作ってからフィールドを埋める初期化や、検証を迂回する復元を禁止します。状態変更を宣言済みの業務操作または明示的なイベント適用に限定する規約と、domain/use-caseからのgetter呼び出し制限も維持します。
