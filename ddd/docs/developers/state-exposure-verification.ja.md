...
CompilerHost は渡したソースと固定した TypeScript パッケージ内の `lib.*.d.ts` だけを読みます。プロジェクト探索、ambient `@types`、外部import・再公開の解決は追加しません。空オブジェクトの設定と、要求と整合する認識済みのプロジェクト設定payloadはどちらも抽出へ進み、それ以外の設定は未解決になります。要求には全設定と実抽出器・解析ライブラリの版を含めます。
