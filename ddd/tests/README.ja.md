...
更新: 2026-09-13。プラグインルートで `bun test tests/` を実行します。言語統一後は285成功・1skip・20失敗です。skipは標準側の単独完了不足を調べる任意実行の再現ケースです。失敗する旧テストのために、削除済み参照サブモジュールや旧bridgeを復元する運用には戻しません。
