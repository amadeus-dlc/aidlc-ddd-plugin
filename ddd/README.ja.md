...
全テストは `bun run check` です。言語統一後は285成功・1skip・20失敗で、失敗は旧環境依存に集中しています。`build:all`、`test:sandbox`、引数なしの `test:dist` はClaude/Codexだけを対象にします。`bun run test:sandbox` はビルド、一時環境へのcompose、配布物の直接検査、通常承認開始の統合検査をまとめて実行します。
