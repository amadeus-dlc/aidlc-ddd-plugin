...
- **公開メンバーの規則 `a` と getter の規則 `d` を、ネイティブ抽出器で判定するようにした。** これらを宣言する `ddd-rust-domain` と `ddd-rust-use-case` は、新しい `domain-facts/1` protocol（`--domain-facts-version`、`protocol_version` 4）を、検査対象プログラムの全ソースを含む1バッチで読む。**公開タプルメンバーを指摘するようになった。** `pub struct Invoice(pub u64);` と、その `pub(crate)`・`pub(super)`・`pub(in ...)` 形式は、メンバーの序数を名として、可視性が始まる行で規則 `a` を報告する。隣にある非公開メンバーは報告しない。**本体が明示的な `return self.amount;` の getter も規則 `d` の対象になり**、その呼出行で報告する。raw識別子は `r#` を保ち、非ASCII名も綴りを保つため、`r#total` が別の型の `total` に吸収されることはない。名前付きフィールド、末尾式の getter、trait 実装、enum のバリアントの合否は従来どおり。
- **この事実を読めないゲートは、合格させずに停止するようにした。** 両ゲートは評価の前に抽出器の起動を1回分類する。ファイルの事実が得られない条件（起動時の6条件、実行が完了しない、protocol 外の応答、リクエスト上限を超えるバッチ）では、verdict を出力せず終了コード 127 で終わる。解析器が受理できなかったファイルはメンバーの記録自体を持たない。規則 `a`・`d` が判定する申告ファイルを持つ実行で、これらの規則の判定元となるファイルにそれが含まれる場合は、同じ終了コードで、抽出器が返した理由とともにゲートを停止する。「公開メンバーを宣言していない」とは報告しない。`ddd-rust-interface-adapter` はどちらの規則も宣言せず、抽出器を起動しない。
- 規則 `a`・`d` のそれ以外の部分は tree-sitter のまま維持する。対象ファイルの集合、型の解決、呼び出し箇所、リポジトリ引数の例外判定が該当する。項目マクロと `cfg`／`cfg_attr` は `domain-facts.unresolved` の note として判定に添える。規則 `a`・`d` が判定する申告ファイルを持つ実行では、受理できなかったファイルも、これらの規則の判定元となるファイルに含まれない場合は同じく note として添え、含まれる場合は上記のとおりゲートを停止する。[残る境界](docs/developers/rust-syn-spike.ja.md)を参照。
