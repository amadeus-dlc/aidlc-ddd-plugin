---
name: aidlc-plugin-dev
generated-by: aidlc-runner-gen
description: >
  Run the AI-DLC workflow with the plugin-dev scope baked in — no scope
  detection. AI-DLC v2 プラグイン開発 — contributions・manifest・tests を設計付きで作り、validate/build で検証する. Packaging over `/skill:aidlc --scope plugin-dev`, which works
  without this skill.
argument-hint: "[description | --status | --stage <slug|#> | --phase <name|#>]"
user-invocable: true
---

# AI-DLC — plugin-dev scope

Drive the AI-DLC engine with the **plugin-dev** scope fixed. This is the same
deterministic forwarding loop the `/skill:aidlc` orchestrator runs, with `--scope
plugin-dev` baked into the first `next` so scope detection is skipped. The
...
1. `directive = bun .kimi-code/tools/aidlc-orchestrate.ts next --scope plugin-dev $ARGUMENTS`
...
Pass `$ARGUMENTS` through verbatim after `--scope plugin-dev`; the engine parses
...
- **Default to CONTINUATION.** Treat the input as new-work ONLY when it clearly
  ...
  (`bun .kimi-code/tools/aidlc.ts engine intent list --json` gives its `slug` and
...
- **On genuine new-work, OFFER, never auto-create.** Surface an
  ...
  `plugin-dev` (the new work is likely the same flavour that made the user reach for
...
- **On CONFIRM**, re-run `next` with `--new-intent`, the confirmed scope, and the
  ...
  ```bash
  bun .kimi-code/tools/aidlc.ts engine orchestrate next --new-intent --scope <the confirmed scope> "<the new-work description>"
  ```
