...
`bun .kimi-code/tools/aidlc.ts engine orchestrate report --stage reverse-engineering --result skipped --reason "<reason>"`.
...
```
bun .kimi-code/tools/aidlc.ts engine workspace codekb-scope-diff --repo <repo>
```
...
- If every repo is reused on an ordinary workflow run, report the stage as
  ...
  `bun .kimi-code/tools/aidlc.ts engine orchestrate report --stage reverse-engineering --result skipped --reason "codekb reuse: all resolved stores CURRENT, human chose reuse"`.
...
its output verbatim:
   ...
   ```
   bun .kimi-code/tools/aidlc.ts engine workspace codekb-scope-diff --repo <repo> --mint --paths <analyzed paths>
   ```
...
```
bun .kimi-code/tools/aidlc.ts engine workspace codekb --repo <repo>
```
...
```
bun .kimi-code/tools/aidlc.ts engine workspace codekb-scope-diff --repo <repo> --compare <record>/inception/reverse-engineering/scope-draft-<repo>.md
```
...
`bun .kimi-code/tools/aidlc.ts engine orchestrate report --stage reverse-engineering --result <outcome>`.
