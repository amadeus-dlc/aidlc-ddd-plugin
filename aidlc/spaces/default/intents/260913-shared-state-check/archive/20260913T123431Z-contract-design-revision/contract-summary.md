...
3. U2は実行の結果をExtractionExecutionへ変換する。completedのresponseは未知の値として受け渡し、U1がEvidenceResponseへ検証する。未知のネイティブ応答を単なる型アサーションで共通応答へ変えない。
4. U1は要求と実行状態の外枠を検証する。この呼出し自体が不正ならinput-rejectedとし、issuesを一件以上返す。対象や実行状態を推測してInspectionResultを作らない。
5. 有効な要求と実行状態の外枠に対して、responseが欠落・不正・未知版・識別不一致なら、元の要求に結び付くevaluated / unresolvedを返す。completedという観測済みの実行状態を、応答不正だけを理由にfailedへ書き換えない。
...
| 入力の状況 | executionState | ruleResult | findings / unresolvedReasons |
|---|---|---|---|
| 完全な一覧、すべてのFactが確定、公開なし | completed | pass | ともに空 |
| 完全な一覧、すべてのFactが確定、公開あり | completed | violation | 公開の所見あり、未解決理由なし |
| 対象未解決、partial、または一つでもFactが未解決 | completed | unresolved | 独立に確定した公開の所見と、未解決理由を保持 |
| responseが欠落・不正・未知版・識別不一致 | completed | unresolved | 所見は空、応答の拒否理由あり |
| 解析器を起動できない | unavailable | unresolved | 所見は空、tool-unavailable等の理由あり |
| 実行失敗・期限切れ・出力制限超過 | failed | unresolved | 所見は空、対応する理由あり |
...
Findingはresolved / trueのメンバーからのみ生成する。所見はmemberId順、各evidenceはfile・byteStart・byteEnd順、理由はcode・subject・locationのfile・byteStart・byteEnd・message順で決定的に整列する。文字列比較はUnicodeスカラー値順、nullの位置は非nullより前とする。入力配列の走査順を結果の意味にしない。

unresolvedReasonsには必ず一件以上の理由がある。passとviolationでは未解決理由を空とする。今回の指定型に対する契約はnot-applicableを持たず、適用外を使う将来の契約では別途、適用外の証明条件と版を定める。
...
| 証跡の例 | 規則結果 | 保持内容 |
|---|---|---|
| complete、privateな保持状態がresolved / false、操作メソッドが根拠付きabsent | pass | 公開の所見なし、未解決理由なし |
