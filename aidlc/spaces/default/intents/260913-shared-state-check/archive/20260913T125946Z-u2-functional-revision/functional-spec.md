...
3. 実行に必要なツールと版を確認する。Rust試作の準備は明示的な準備コマンドに分け、検証中の依存ダウンロードや無制限ビルドをしない。TypeScript側の標準ライブラリも、記録した解析器版に結び付く準備済み資産を使う。
...
1. 解析器を終了させられるプロセスまたはワーカー境界で起動し、C2の期限と出力上限を適用する。起動できなければunavailable、起動後の異常終了・期限・出力超過ならfailedとする。
2. 終了を待ち、出力の個数・版・形状を検証する。途中のJSON、複数応答、要求が異なる応答を採用しない。待機だけを打ち切って解析が動き続ける実装にしない。
3. completedで応答がない場合はresponse:nullとする。存在するが解釈できない出力も、正常な空の共通証跡に変えない。共通形式を成立させられない場合はC1がinvalid-responseとして扱う値へ対応付け、ネイティブの診断は実行記録に残す。
...
既存のRust試作の準備を独立した明示コマンドとして案内し、検証入口は準備済みの資産だけを使う。具体的な内部関数、型の実装、固定ケースファイル、プロセス管理コード、実行スクリプトはコード生成の計画で確定する。共通契約の変更が必要ならU1への影響を明示し、U2だけで契約の意味を変えない。
