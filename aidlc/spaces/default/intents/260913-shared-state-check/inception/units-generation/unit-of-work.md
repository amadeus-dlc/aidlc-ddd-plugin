...
- 異常を意図的に与える試験では、規則結果だけでなく、期待した違反・未解決理由・根拠の対応を照合する。別の不正が先に拒否され、期待と異なる理由で検査不能となった場合も不一致とする。想定どおりの非正常結果と、試験実行そのものの失敗を区別する。
...
| Story ID | 主担当 | 主担当の責務 | 単位をまたぐ担当 |
|---|---|---|---|
| US1.1: 指定Rust型の状態公開を確認する | U2 | RustStateEvidenceの対象同定・抽出・変換と、StateExposureVerificationによる実行 | U1が要求・根拠を検証し、共通判定を提供する |
| US1.2: 指定TypeScript型を同じ基準で確認する | U2 | TypeScriptStateEvidenceの二表現の抽出・変換と、検証実行 | U1がRustと同じ意味の契約・判定を提供する |
| US1.3: 確定違反と検査不能を区別する | U1 | StateExposureInspectionによる契約検証・完全性の確認・判定 | U2が実際の根拠・実行状態を渡し、結果・確定違反・未解決理由を区別して報告する |
| US1.4: 再実行して回帰を検出する | U2 | 実行・期待値比較・自動試験への登録・既存Rust回帰・英日での実行案内 | U1が契約・判定の独立試験と仕様を提供し、U2が両単位の型検査・結合検証を行う |

主担当は成果全体の唯一の実装者を意味しない。受入条件を満たす責務と協力範囲は[対応表](unit-of-work-story-map.md)で追跡し、各構成要素の所有はUnit Catalogueの割当てを維持する。
