# Unit Test Instructions — U4 設計センサー（u4-design-sensors）

## この文書の範囲

本 Unit（`u4-design-sensors`）のテストの実行方法だけを定める。他の Unit のテストは含めない。
...
```sh
cd ddd && bun test tests/u4-design-sensors.test.ts
...
```sh
cd ddd && bun test tests/u4-golden.test.ts --test-name-pattern "violation-k"
cd ddd && bun test tests/u4-design-sensors.test.ts --test-name-pattern "layer-structure"
```

本 Unit のテストファイルは `ddd/tests/u4-design-sensors.test.ts`（17 件）と
`ddd/tests/u4-golden.test.ts`（ゴールデンケース 40 件＋規約テスト 3 件）の 2 本である。
`ddd/tests/golden/runner.ts` と `ddd/tests/golden/design/cases.ts` は本 Unit が所有し、
U5 が `tests/golden/rust/` に対して同じランナーを使う。
...
- **戦略**: Standard — コンポーネントあたり 5〜8 件。本 Unit のコンポーネントは
  DesignSensorSuite（6 センサー）と GoldenCaseSuite（規約と設計センサー分）。
- **スコープ床（plugin-dev）**: 追加の新規テスト床は無い。既存スイートが緑であることのみ。
- **実測の目安**: 6 センサーそれぞれに正常系と違反系が少なくとも 1 件ずつあり、
  ゴールデンケースが各マニフェストの宣言する rule_id を 1 件以上の違反ケースで覆い、
  決定性（3 回実行して一致）と起動契約（1 行 JSON verdict）が踏まれていること。
...
- **ネットワーク・外部プロセスを呼ばない。** 本 Unit のテストはファイルを読み書きするだけである。
  対象は一時ディレクトリに実体化した疑似記録ディレクトリで、ネットワークにも
  `aidlc-workflows` にも触れない。
...
- **正規モデルはモックしない。** U1 の `loadDomainModel` を実物のまま通す。
  モデルの読み込み失敗は U1 の所見を転記する経路そのものが検証対象だからである。
- **ビルド成果物を読まない。** `dist/` は `.gitignore` の管理外であり、内容はビルドに依存する。
  テストは `dist/` を参照しない（`bun run test:dist` は別の関心事で、本 Unit のテストではない）。
- 時刻・乱数に依存する分岐を持たない。同一の作業ツリーに対して同一の結果が得られる。

## テストデータの管理

- **フィクスチャは `tests/` 配下に置く。** `tools/` には置かない（BR8.6）。
  設計ケースの実体は `ddd/tests/golden/design/cases.ts` の表にあり、
  ケースごとに一時ディレクトリへ実体化される。fixture のソースは書き換えない。
- **一時ディレクトリは必ず後始末する。** ランナーは `finally` で `rmSync` し、
  テストは `afterEach` で作った分をすべて削除する。
- **所見のパスは記録ディレクトリ相対で比較する。** 記録ディレクトリが
  設計成果物の固定した座標系であるため、ゴールデンケースは安定したパスで比較できる。
...
- マニフェストの宣言とスクリプトの発行する rule_id が食い違う場合、どちらが正しいかは
  `functional-spec.md` §1 と `rules.md` の BR1 に照らして判断する。
